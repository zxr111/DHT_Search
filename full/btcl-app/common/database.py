...
class ElasticsClient:
    ...
    def Index_Data(self, list, infohash):
        ...
        for item in list:
            ...
            if(res['_shards']['successful'] == 1):
                v = eval(RedisClients1.getValue(infohash))
                if res['_version'] > v['hot']:
                    v['hot'] = res['_version']
                self.logger.info("种子入库ES成功 >>>> {0} - 热度： {1}".format(infohash, res['_version']))
    ...
    def Get_Data_By_Body(self, value, _from, pageSize):
        # doc = {'query': {'match_all': {}}}
        doc = {
                "query": {
                    "dis_max": {
                        "queries": [
                            { "match_phrase": { "bare_name": value }}, #对标题进行词匹配
                            #{ "match_phrase": { "file_list": value }}
                        ],
                        "tie_breaker": 0.3
                    },
                },
